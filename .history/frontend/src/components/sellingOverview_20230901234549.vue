...
<template>
    <div class="outer-container">
        ...
        <div class="item-list">
            ...
            <div v-if="itemList != -1 && dropdownList != -1 && selectedCategory == 'category'" v-for="item in itemList" :key="item.id" class="user-item" @click="goToItemPage(item.id)">
                <img :src="item.picture" alt="picture" class="item-card-img" >
                <div class="item-listing">
                    <h2 class="item-title">{{ item.title }}</h2>
                    <p class="item-price">£{{ item.starting_price }}</p>
    ...
    setup(props, context){
        ...
        const selectCategory = async(code) =>{
            ...
            try{
                ...
                console.log("dropdown is", dropdownList.value.iid);
                console.log("category, ", categoryItems.value);
