    ...
    <div class="outer-container">
        ...
        <div class="page-container">
            ...
            <div class="itempage-right">
                ...
                <!--this should all be computed instead-->
        ...
        <div class="itempage-center">
            <div class="page-container">
                <div class="itempage-left">
                    <img :src="sellerInfo.image"  class="profile-pic">
                    <p> {{ sellerInfo.email }}</p>
                    <button> seller's other items</button>
                </div>
                ...
                <div class="itempage-right">
                    <form class="form-card" @submit.prevent>
                        <h2>send question</h2>
                        <textarea class="text-field" v-model="question"></textarea>
...
export default defineComponent({
    ...
    setup(props, context){
        ...
        const calculateTime = async() =>{
            if (aunctionHasEnded.value || !isCurrentRouterView.value){
        ...
        const getHighestBid = async() => {
            ...
            try{
                ...
                console.log("response", response.data );
                console.log("highest", response.data.highest_bid, typeof response.data.highestBid);
                console.log("current", item.value.starting_price);
                if (response.data.highest_bid > item.value.starting_price){
                    console.log("assied topp price");
        ...
        //this iis still running when navigating away 
        const checkAuctionEnd = async() => {
            const now = new Date();
            console.log("now", now, "auction end", auction_end.value);
            if (!isCurrentRouterView.value){
                ...
                //i need to now update the item instance so that field sold = true
                cancelAnimationFrame(checkAuctionEnd);
                return;
            }
            else{
