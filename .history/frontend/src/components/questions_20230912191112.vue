            ...
            <div class="itempage-right">
                <div>
                    ...
                    <div v-for="(question, index) in allQuestions" :key="question.id">
                        ...
                        <p v-if="answers[index]"> A: {{ getAnswer(index) }} </p>
                        <p v-else>Not available</p>
                        <AnswerQ  :question_id="question.id" :sellerInfo="sellerInfo" @answered="handleNewAnswer"/>
                    </div>
                    -->
                    <div v-for="(question, index) in answeredQuestions" :key="question.id">
                        <h4>Q: {{ question.question }}</h4>
                        <p v-if="question.answer !== null">A: {{ question.answer }}</p>
                        <AnswerQ :question_id="question.id" :sellerInfo="sellerInfo" @answered="handleNewAnswer" />
                    </div>
                </div>
            </div>
...
export default defineComponent({
    ...
    setup(props, context){
        ...
        const answeredQuestion = computed(() => {
            return this.allQuestions.map((question, index) => ({
                ...question,
