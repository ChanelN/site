...
<template>
    <div class="outer-container" v-if="items.length >0">
        ...
        <div class="dropdown-wrapper" v-if="items.length >0">
        ...
        so maybe first look for all active items in that category by user, and then in frontend filter based on if its in the selectedOption-->
            ...
            <div class="item-list">
                ...
                <div v-if="itemList != -1  && dropdownList == 0 && selectedCategory == 'category'" v-for="item in itemList" :key="item.id" class="user-item" @click="goToItemPage(item.id)">
                    <img :src="item.picture" alt="picture" class="item-card-img" >
                    <div class="item-listing">
                        <h2 class="item-title">{{ item.title }}</h2>
                        <p class="item-price">£{{ item.starting_price }}</p>
    ...
    <div class="outer-container">
        ...
        <!--keep history of all their bids, seperated by item-->
        <div class="item-list">
            <div v-if="AllBids" v-for="bid in AllBids" :key="bid.id" class="user-item">
                <div class="item-listing">
                    <h2 class="item-title" v-if="bid.won ==true"> Current highest bidder for: {{ bid.item_readable}}</h2>
                    <p class="item-price"> {{ bid.bid_price }}</p>
                    <p> {{ bid.bid_time }}</p>
                </div>
            </div>
        </div>
        ...
        <h3> My purchases: </h3>
        <!-- Item model has the bidder_id as well as is_sold=True (meaning that the auction has ended with the bidder_id as the final winner)-->
        <div class="item-list">
            <div v-if="purchasedItems" v-for="item in purchasedItems" :key="item.id" class="user-item" @click="goToItemPage(item.id)">
                <img :src="item.picture" alt="picture" class="item-card-img" >
                <div class="item-listing">
                    <h2 class="item-title">{{ item.title }}</h2>
                    <p class="item-price">£{{ item.starting_price }}</p>
...
export default defineComponent({
    ...
    setup(props, context){
        ...
        const getUserItems = async() =>{
            //this gets all items MADE by user e.g creator
            try{
        ...
        const getPurchasedItems = async() => {
            //this gets all items BROUGHT by user e.g winner_id
            console.log("id of current user is, ", user.value.id);
            try{
                const response = await axios({
                    method: 'get',
                    url: `http://127.0.0.1:8000/item/won/${user.value.id}/`,
                    headers: {
                        'X-CSRFToken': csrfToken,
                        'X-SessionID': sessionId,
                        'Content-type': 'application/JSON',
        ...
        const getAllBids = async() => {
            const response = await axios({
                method: 'get',
                url: "http://127.0.0.1:8000/bidding/all-bids/",
                headers: {
                        'X-CSRFToken': csrfToken,
                        'X-SessionID': sessionId,
                        'Content-type': 'application/JSON',
