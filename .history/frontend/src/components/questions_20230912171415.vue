            ...
            <div class="itempage-right">
                <div>
                    ...
                    <div v-for="(question, index) in allQuestions" :key="question.id">
                        ...
                        <!-- v-if="answers[index] && answers[index].length > 0" -->
                        <p> A: {{ getAnswer(question.id)  }}</p>
...
export default defineComponent({
    ...
    setup(props, context){
        ...
        const getAnswer = async(id) => {
            ...
            const object = JSON.stringify(answers.value[questionIndex]);
            const { answer } = object;
            console.log("answer ", answer);
            
            console.log("answers is", JSON.stringify(answers.value[questionIndex].answer));
            return JSON.stringify(answers.value[questionIndex]);
            //return JSON.stringify(answers.value);
            //return response.data.map((answer) => answer.answer).join(', ');
        };
