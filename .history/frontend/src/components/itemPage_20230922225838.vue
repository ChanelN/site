...
export default defineComponent({
    ...
    setup(props, context){
        ...
        const calculateTime = async() =>{
            ...
            if ( now >= auction_end.value|| !isCurrentRouterView.value){
        ...
        const updateBidWinner = async() => {
            ...
            // if you go onto the page as the buyer of item, it can't update since its already 'sold' to THIS BUYER
            console.log("UPDATE winner_id", winner_id.value);
            try{
                if(winner_id.value){
                    const response = await axios({
                        method: 'put',
        ...
        const findWinnerEmail = async() => {
            //need to change this to a more secure URL - my users/ is public but has confidential info - can i blur?
            const email = await axios({
                method: 'get',
                url: `http://127.0.0.1:8000/users/${winner_id.value}/`,
                withCredentials: true,
            })
            console.log("find email", email.data.email);
            winner_email.value = email.data.email;
        }

        const sendWinnerEmail = async() => {
            //view to send is in Item
            /*
            The signin component now verifies that the email belongs to legit account so i can send emails
