...
<template>
    ...
    <div class="outer-container">
        ...
        <div v-if="AllBids" v-for="bid in AllBids" :key="bid.id" class="user-item">
            <h2 class="item-title"> {{ bid.item }}</h2>
            <p> {{ bid.bid_price }}</p>
            <p> {{ bid.bid_time }}</p>
            <p v-if="bid.won==true && bid.item not in purchasedItems"> You're currently the highest bidder</p>
        </div>
        <!---->
        <h3> My purchases: </h3>
        ...
        <div class="item-list">
            <div v-if="purchasedItems" v-for="item in purchasedItems" :key="item.id" class="user-item">
                <img :src="item.picture" alt="picture" class="item-card-img" >
                <div class="item-listing">
                    <h2 class="item-title">{{ item.title }}</h2>
                    <p class="item-price">£{{ item.starting_price }}</p>
